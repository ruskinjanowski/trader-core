...
public interface LunoAuthenticated extends Luno {
	...
	/**
	 * Create an additional account for the specified currency. You must be verified
	 * to trade currency in order to be able to create an account. A user has a
	...
	@POST
	@Path("accounts")
	LunoAccount createAccount(@HeaderParam("Authorization") ParamsDigest auth, @QueryParam("currency") String currency,
			@QueryParam("name") String name) throws IOException, LunoException;
	 ...
	 * the oldest entry. The range of rows to return are specified with the min_row
	 * (inclusive) and max_row (exclusive) parameters. At most 1000 rows can be
	 * requested per call.<br>
	 * If min_row or max_row is non-positive, the range wraps around the most recent
	 * row. For example, to fetch the 100 most recent rows, use min_row=-100 and
	 * max_row=0.
	 *
	 * @param auth
	...
	LunoAccountTransactions transactions(@HeaderParam("Authorization") ParamsDigest auth, @PathParam("id") String id,
			@QueryParam("min_row") int minRow, @QueryParam("max_row") int maxRow) throws IOException, LunoException;
	...
	LunoPendingTransactions pendingTransactions(@HeaderParam("Authorization") ParamsDigest auth,
			@PathParam("id") String id) throws IOException, LunoException;

	/**
	 * Returns a list of the most recently placed orders. You can specify an
	 * optional state=PENDING parameter to restrict the results to only open orders.
	 * You can also specify the market by using the optional pair parameter. The
	...
	@GET
	@Path("listorders")
	LunoOrders listOrders(@HeaderParam("Authorization") ParamsDigest auth, @QueryParam("state") State state,
			@QueryParam("pair") String pair) throws IOException, LunoException;

	/**
	 * Create a new trade order.<br>
	 * Warning! Orders cannot be reversed once they have executed. Please ensure
	 * your program has been thoroughly tested before submitting orders.<br>
	 ...
	 * IDs by calling the Balances API
	...
	@POST
	@Path("postorder")
	LunoPostOrder postLimitOrder(@HeaderParam("Authorization") ParamsDigest auth, @FormParam("pair") String pair,
			@FormParam("type") OrderType type, @FormParam("volume") BigDecimal volume,
			@FormParam("price") BigDecimal price, @FormParam("base_account_id") String baseAccountId,
			@FormParam("counter_account_id") String counterAccountId, @FormParam("post_only") boolean postOnly)
	 ...
	 * currency or counter currency account will be used. You can find your account
	 * IDs by calling the Balances API
	 * https://www.luno.com/en/api#accounts-balances.<br>
	 * A market order executes immediately, and either buys as much bitcoin that can
	 * be bought for a set amount of fiat currency, or sells a set amount of bitcoin
	...
	@POST
	@Path("marketorder")
	LunoPostOrder postMarketOrder(@HeaderParam("Authorization") ParamsDigest auth, @FormParam("pair") String pair,
			@FormParam("type") OrderType type, @FormParam("counter_volume") BigDecimal counterVolume,
			@FormParam("base_volume") BigDecimal baseVolume, @FormParam("base_account_id") String baseAccountId,
	...
	@POST
	@Path("stoporder")
	LunoBoolean stopOrder(@HeaderParam("Authorization") ParamsDigest auth, @FormParam("order_id") String orderId)
			throws IOException, LunoException;
	 ...
	 * first.<br>
	 * type in the response indicates the type of order that you placed in order to
	 * participate in the trade.<br>
	 * If is_buy in the response is true, then the order which completed the trade
	...
	@GET
	@Path("listtrades")
	LunoUserTrades listTrades(@HeaderParam("Authorization") ParamsDigest auth, @QueryParam("pair") String pair,
			@QueryParam("since") Long since, @QueryParam("limit") Integer limit) throws IOException, LunoException;
	...
	@GET
	@Path("fee_info")
	LunoFeeInfo feeInfo(@HeaderParam("Authorization") ParamsDigest auth, @QueryParam("pair") String pair)
			throws IOException, LunoException;
	...
	LunoFundingAddress getFundingAddress(@HeaderParam("Authorization") ParamsDigest auth,
			@QueryParam("asset") String asset, @QueryParam("address") String address) throws IOException, LunoException;

	/**
	 * Allocates a new receive address to your account. There is a rate limit of 1
	 * address per hour, but bursts of up to 10 addresses are allowed.
	 *
	 * @param auth
	 * @return
	 * @throws IOException
	...
	LunoFundingAddress createFundingAddress(@HeaderParam("Authorization") ParamsDigest auth,
			@FormParam("asset") String asset) throws IOException, LunoException;
	...
	Withdrawal requestWithdrawal(@HeaderParam("Authorization") ParamsDigest auth, @FormParam("type") String type,
			@FormParam("amount") BigDecimal amount, @FormParam("beneficiary_id") String beneficiaryId)
			throws IOException, LunoException;
	...
	@GET
	@Path("withdrawals/{id}")
	Withdrawal getWithdrawal(@HeaderParam("Authorization") ParamsDigest auth, @PathParam("id") String withdrawalId)
			throws IOException, LunoException;

	/**
	 * Cancel a withdrawal request. This can only be done if the request is still in
	 * state PENDING.
	 *
	 * @param auth
	 ...
	 * invitation to create an account and claim the funds will be sent.<br>
	 * Warning! Bitcoin transactions are irreversible. Please ensure your program
	...
	@POST
	@Path("send")
	LunoBoolean send(@HeaderParam("Authorization") ParamsDigest auth, @FormParam("amount") BigDecimal amount,
			@FormParam("currency") String currency, @FormParam("address") String address,
			@FormParam("description") String description, @FormParam("message") String message)
			throws IOException, LunoException;
	 ...
	 * amount that you want too receive.<br>
	...
	LunoQuote createQuote(@HeaderParam("Authorization") ParamsDigest auth, @FormParam("type") OrderType type,
			@FormParam("base_amount") BigDecimal baseAmount, @FormParam("pair") String pair)
			throws IOException, LunoException;
	...
	@GET
	@Path("quotes/{id}")
	LunoQuote getQuote(@HeaderParam("Authorization") ParamsDigest auth, @PathParam("id") String quoteId)
			throws IOException, LunoException;
	...
	@PUT
	@Path("quotes/{id}")
	LunoQuote exerciseQuote(@HeaderParam("Authorization") ParamsDigest auth, @PathParam("id") String quoteId)
			throws IOException, LunoException;
	...
	@DELETE
	@Path("quotes/{id}")
	LunoQuote discardQuote(@HeaderParam("Authorization") ParamsDigest auth, @PathParam("id") String quoteId)
			throws IOException, LunoException;
}
