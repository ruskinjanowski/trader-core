...
public class LunoTradeService extends LunoBaseService implements TradeService {
	...
	public LunoPendingTransactions pendingTransactions(String id) throws IOException, LunoException {
		return lunoAPI.pendingTransactions(id);
	}
	...
	public OpenOrders getOpenOrders(OpenOrdersParams params) throws ExchangeException,
		...
		for (org.knowm.xchange.luno.dto.trade.LunoOrders.Order lo : lunoAPI.listOrders(State.PENDING, null)
				.getOrders()) {
			list.add(new LimitOrder(convert(lo.type), lo.limitVolume, LunoUtil.fromLunoPair(lo.pair), lo.orderId,
					lo.getCreationTimestamp(), lo.limitPrice));
		}
	...
	public Collection<Order> getOrder(String... orderIds) throws IOException {
		...
		for (String id : orderIds) {
			org.knowm.xchange.luno.dto.trade.LunoOrders.Order lo = lunoAPI.getOrder(id);
			LimitOrder limitOrder = new LimitOrder(convert(lo.type), lo.limitVolume, LunoUtil.fromLunoPair(lo.pair),
					lo.orderId, lo.getCreationTimestamp(), lo.limitPrice);
			limitOrder.setOrderStatus(convertState(lo.state));
			limitOrder.setCumulativeAmount(lo.base);
			limitOrder.setFee(lo.feeBase);
			limitOrder.setAveragePrice(lo.limitPrice);
	...
	public String placeMarketOrder(MarketOrder marketOrder) throws IOException {
		...
		LunoPostOrder postOrder = marketOrder.getType() == OrderType.ASK
				? lunoAPI.postMarketOrder(LunoUtil.toLunoPair(marketOrder.getCurrencyPair()),
						org.knowm.xchange.luno.dto.trade.OrderType.SELL, null, marketOrder.getOriginalAmount(), null,
						null)
				: lunoAPI.postMarketOrder(LunoUtil.toLunoPair(marketOrder.getCurrencyPair()),
	...
	public String placeLimitOrder(LimitOrder limitOrder) throws IOException {
		LunoPostOrder postLimitOrder = lunoAPI.postLimitOrder(LunoUtil.toLunoPair(limitOrder.getCurrencyPair()),
				convertForLimit(limitOrder.getType()), limitOrder.getOriginalAmount(), limitOrder.getLimitPrice(), null,
				null);
	...
	public UserTrades getTradeHistory(TradeHistoryParams params) throws ExchangeException, IOException {
		...
		for (org.knowm.xchange.luno.dto.trade.LunoUserTrades.UserTrade t : lunoTrades.getTrades()) {
			...
			trades.add(new UserTrade(t.buy ? OrderType.BID : OrderType.ASK, t.volume, pair, t.price, t.getTimestamp(),
					tradeId, t.orderId, feeAmount, feeCurrency));
		}
	...
	public static OrderStatus convertState(State state) {
		switch (state) {
		case PENDING:
